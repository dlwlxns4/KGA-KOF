...
public:
	...
	typedef struct tagImageInfo
	{
		...
		// 애니메이션 용 데이터
		int maxFrameX;	//x프레임 갯수
		int maxFrameY;	//y프레임 갯수
		int frameWidth;	//한프레임 크기
		int frameHeight;
		int currFrameX;	//현재인덱스
		int currFrameY;
...
public:
	...
	void Render(HDC hdc, int destX, int destY, int hp, bool isPlayer1);
	void Render(HDC hdc, int destX, int destY, int x);
